...
#####Niche######
spearman_r <- read.csv("Gene Abundance_Niche_Spearman r.csv",header = T,row.names = 1)
spearman_p <- read.csv("Gene Abundance_Niche_Spearman p.csv",header = T,row.names = 1)
...
LMM <- read.csv("Niche_LMM result.csv",header = T,row.names = 1)
LMM <- LMM[c("Niche2.mean","Niche2.P"),]
...
# Safely remove Anammox_Bacteria rows
...
topanno <- HeatmapAnnotation(
  Nitrogen = group,  # Directly use named character vector
  col = list(
    Nitrogen = c(
      'N0' = '#ABD3F1',
      'N2' = '#80A6D7',
...
pdf("Niche_Spearman_LMM.pdf", width = 6, height = 8)
draw(p)
dev.off()