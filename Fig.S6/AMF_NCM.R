#############################################################
###### AMF_Niche_neutral community model #############
rm(list = ls())
library(Hmisc)
library(minpack.lm)
library(stats4)
...
spp <- read.csv('AMF_ASV_rarefaction.csv', head = TRUE, stringsAsFactors = FALSE, row.names = 1)
