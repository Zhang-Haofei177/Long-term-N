rm(list = ls())
setwd("C:/Users/ADMIN/Desktop/Longterm N code/Fig.S5/AMF/")

####### Ori compartment ##########
niche <- "Ori"
metadata <- read.table(paste0("mappingAMF_", niche, ".txt"), sep = '\t', h = TRUE, row.names = 1, check.names = FALSE, comment.char = '')

otus <- read.table('AMF_ASVtable.txt', sep = '\t', header = TRUE, row.names = 1, check.names = FALSE, skip = 1, comment.char = '')
otus <- round(otus)
...
write.csv(combined_result, "AMF source track result.csv")